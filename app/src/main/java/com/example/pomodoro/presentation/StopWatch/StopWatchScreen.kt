...
fun StopwatchScreen(
    ...
    viewmodel: StopWatchViewmodel = hiltViewModel()
       ...
       StopwatchScreenContent(modifier = modifier, state = state,viewmodel::onPaused,viewmodel::onResumed)

}

@Composable
fun StopwatchScreenContent(modifier: Modifier ,state :StopwatchScreenState,onPause:() -> Unit,onResumed:(Long) -> Unit) {

    val duration:Long = state.taskitem?.remaining_time?.times(1000)?.toLong() ?: 25000L
    ...
    Column(modifier.padding(20.dp).fillMaxSize(), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
       StopwatchTimerComponent(modifier = Modifier.size(500.dp).align(Alignment.CenterHorizontally), time =TimeFormatter.longtoTime(time), progress = progress)
